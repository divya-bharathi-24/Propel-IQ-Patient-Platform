...
2. Builds the .NET 9 API Docker image, pushes it to a container registry (GitHub Container Registry — free), and deploys to Railway via the Railway CLI.
...
| `server/Dockerfile` | VERIFY | Must exist and produce a runnable .NET 9 API image; CREATE if absent |
...
| CREATE (if absent) | `server/Dockerfile` | Multi-stage .NET 9 Docker image for Railway deployment |
...
- [ ] Verify `server/Dockerfile` exists; if absent, create multi-stage .NET 9 Dockerfile (`sdk:9.0` build → `aspnet:9.0` runtime)
