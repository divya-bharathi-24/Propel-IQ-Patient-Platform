...
- **Node.js**: `>= 20.0.0` (LTS recommended — verify with `node --version`)
- **npm**: `>= 10.0.0` (bundled with Node.js 20)
- **Angular CLI**: `^18.x` — install globally with `npm install -g @angular/cli@18`

### Getting Started (Frontend)

The Angular 18 SPA lives in the `app/` directory.
...
Open your browser at [http://localhost:4200](http://localhost:4200). The app will reload automatically on file changes.

### Port Configuration

The default port is **4200**. To run on a different port:

```bash
# Override port via CLI flag
ng serve --port 4201

# Or set it permanently in angular.json under:
# projects.propel-iq-patient-platform.architect.serve.options.port
```

### Build Commands

```bash
# Production build (LCP-optimized, esbuild)
...
The full platform stack — Angular 18 frontend, ASP.NET Core 10 API, PostgreSQL 16 + pgvector, and Redis 7 — can be started with a single command via Docker Compose.

### Prerequisites

- [Docker Desktop](https://www.docker.com/products/docker-desktop/) 24.x or later (with Docker Compose v2 included)
...
cp .env.example .env
# Edit .env: set POSTGRES_PASSWORD, JWT_SECRET (≥32 chars), etc.
...
3. Seed the Specialty reference table (10 rows) if empty
4. Start serving HTTP traffic on port 5000
...
# Full reset — stop containers and delete volumes
docker compose down -v

# Test graceful degradation: stop Redis only, then verify X-Degraded: redis header
docker compose stop redis
curl -i http://localhost:5000/healthz
```

### Graceful Degradation (AC4 / NFR-018)

If the Redis container is stopped, the remaining services (frontend, backend, postgres) continue running unaffected. Redis-dependent API endpoints return HTTP 200 with an `X-Degraded: redis` response header and serve uncached fallback responses. The `/healthz` endpoint reports `"redis": "degraded"` while `"db": "ok"` remains unchanged.
...
| `create-sprint-plan`          | Generates sprint plans with dependency-ordered story allocation, sprint goals, capacity planning, and critical path analysis                            | epics.md (required), us\_\*.md (required), project_plan.md (recommended), EP-XXX filter, `--sprint-duration`, `--velocity`, `--buffer` flags (optional) | `.propel/context/docs/sprint_plan.md`                                                                 | `/create-sprint-plan`                                                |
...
| `generate-playwright-scripts` | Generates production-ready Playwright TypeScript scripts from test workflow specifications                                                              | tw*\*.md or e2e*\*.md file(s), --type flag                                                                                                              | `test-automation/tests/*.spec.ts`, `test-automation/e2e/*.spec.ts`, `test-automation/pages/*.page.ts` | `/generate-playwright-scripts`                                       |
...
_For detailed prompt documentation, refer to individual prompt files in `.github/prompts/`. For comprehensive instructions details, see files in `.github/instructions/`. For template structures, see files in `.propel/templates/`._
